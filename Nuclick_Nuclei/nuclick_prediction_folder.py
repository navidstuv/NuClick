...
def generateInstanceMap(masks,boundingBoxes,m,n):
    instanceMap = np.zeros((m,n),dtype=np.uint8)
    for i in range(len(masks)):
...
#            print("Contrasted images prediction, DONE!")
    preds /= predNum 
    try:
        masks = postProcessing(preds,thresh=0.5,minSize=100,minHole=100,doReconstruction=True,nucPoints=nucPoints)
    except:
        masks = postProcessing(preds,thresh=0.5,minSize=100,minHole=100,doReconstruction=False,nucPoints=nucPoints)
...
for i in range(len(modelNames)):
    ...
    modelBaseName = 'nuclickHemato_%s_%s%s' % (modelNames[i], losses[i],suffixes[i])
    modelName = 'nuclickHemato_%s_%s' % (modelNames[i], losses[i])
...
path = "E:/Nuclick project_Hemato/Data/Validation_resized/images/"
os.chdir(path)
fileNames = []
...
for fileName in fileNames:
    ...
    pngName = fileName[0:-4]
    if not os.path.isfile(path+fileName):
        instanceMap = np.zeros((512,820),dtype=np.uint8)
