...
################################## RANDOMLY Translate the NucPoint::: JUST A LITTLE ####### For TEST
        if self.random_click_perturb == 'SkeletonValid':
            binaryMask = np.uint8(mask1[:,:,0]>(0.9*np.max(mask1[:,:,0])))
            if np.sum(binaryMask)>30:
                newMask = binaryMask
        ...
        if self.random_click_perturb == 'Skeleton':
            ...
            if np.sum(binaryMask)>30:
                ...
                tempThresh = np.min([tempThresh,np.max(temp)-2])
                newMask = temp>(tempThresh)
                if np.sum(newMask)==0:
                    newMask = binaryMask
                    
                # randomly select one of two skeleton methods
                if np.random.rand()>=0.3:
                    skel = skeletonize_3d(newMask)
                    skel = skel>0 #CONVERTING THE RANGE OF SKEL TO BOOLEAN (standard was in 0,255)
                else:
                    skel = skeletonize(newMask)
