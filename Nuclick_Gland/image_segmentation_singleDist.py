...
def albumentation_transform(x):
    ...
    aug = Compose([
            OneOf([
                ...
                HueSaturationValue(hue_shift_limit=0, sat_shift_limit=(-30,20), val_shift_limit=0, always_apply=False, p=.5),#.8
            ...
            OneOf([
                ...
                p=1.),
            OneOf([
                GaussNoise(var_limit=(5.0, 15.0), p=.4),
...
class ImageDataGenerator(object):
    ...
    def random_transform(self, x,weightMap,mask1,mask2,hm, seed=None):
        ...
        if self.random_click_perturb == 'Train':
            binaryMask = np.uint8(mask1[:,:,0]>=(.5*np.max(mask1)))
            temp = distance_transform_edt(binaryMask)
            thisObject = np.uint8(temp>np.random.randint(0,5))
            ...
            if len(stats)>0:
                ...
                cX = []
                cY = []
                rectImg = np.zeros_like(thisObject)
                #LEFT LINE
                thisLine = rectImg.copy();
                thisLine [bb[0]:bb[2], bb[1]] = 1;
                thisLine = (thisLine*thisObject)>0;
                indices = np.argwhere(thisLine==1) #
                rndIdx  = np.random.randint(0,len(indices))
                cY.append(indices[rndIdx,0])
                cX.append(indices[rndIdx,1])
                
                #RIGHT LINE
                thisLine = rectImg.copy();
                thisLine [bb[0]:bb[2], bb[3]-1] = 1;
                thisLine = (thisLine*thisObject)>0;
                indices = np.argwhere(thisLine==1) #
                rndIdx  = np.random.randint(0,len(indices))
                cY.append(indices[rndIdx,0])
                ...
                rectImg[cY,cX] = 1
                weightMap[:,:,0] = np.float32(rectImg)
            
        if self.random_click_perturb == 'Test':
            cX = []
            cY = []
            labels = bwlabel(weightMap[:,:,0]>0)
            stats = regionprops(labels)
            if len(stats)>0:
                for region in stats:
                    thisY = np.floor(region.centroid[0]+np.random.randint(-5,5))
                    thisY = np.max([0,thisY])
                    thisY = np.min([thisY,mask1.shape[0]])
                    thisX = np.floor(region.centroid[1]+np.random.randint(-5,5))
                    thisX = np.max([thisX,0])
                    thisX = np.min([thisX,mask1.shape[1]])
                    cX.append(np.int(thisX))
                    cY.append(np.int(thisY))
                rectImg = np.zeros_like(labels,dtype=np.float32)
                rectImg[cY,cX] = 1
                weightMap[:,:,0] = rectImg
