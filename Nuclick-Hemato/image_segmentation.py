...
def albumentation_transform(x):
    ...
    aug = Compose([
            ...
            OneOf([
                ...
                p=1.),
            OneOf([
                GaussNoise(var_limit=(3.0, 10.0), p=.5),
                JpegCompression(quality_lower=85, quality_upper=95, p=0.5),
                ], 
                p=1),
    ...
    def random_transform(self, x,weightMap,mask1,mask2,hm, seed=None):
        ...
        if self.elastic_deformation:
            if np.random.random() < .5:
                ...
                if len(np.unique(mask1))>1:
                    mask1 = warp(mask1, tform, output_shape=(IMAGE_HIGHT, IMAGE_WIDTH),preserve_range=True,order=1) 
                if len(np.unique(mask2))>1:
                    mask2 = warp(mask2, tform, output_shape=(IMAGE_HIGHT, IMAGE_WIDTH),preserve_range=True,order=1) 
        ...
        if transform_matrix is not None:
            ...
            x = apply_transform(x, transform_matrix, img_channel_axis,
                                fill_mode=self.fill_mode, cval=self.cval, transOrder=3)
            mask1 = apply_transform(mask1, transform_matrix, img_channel_axis,
                                fill_mode="constant", cval=0, transOrder=1)
...
################################## RANDOMLY Translate the NucPoint::: JUST A LITTLE ####### For TEST
        if self.random_click_perturb == 'Train':
            ...
            newMask = np.float32(temp>3)
            if np.sum(newMask)==0:
                newMask = np.float32(temp>1)
            if np.sum(newMask)==0:
